...
class OAuth2TokenTest extends Test {
    ...
    public function test_array_round_trip(): void {
        ...
        $originalExpiry = $token->getExpiresAt();
        $restoredExpiry = $restored->getExpiresAt();
        $this->assertNotNull($originalExpiry);
        $this->assertNotNull($restoredExpiry);
        $this->assertSame($originalExpiry->getTimestamp(), $restoredExpiry->getTimestamp());
    }
